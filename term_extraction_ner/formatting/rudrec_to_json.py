...
def fulldoc_to_json(input_file, filename, output_file, map_file):
...
def jsondoc_linewise_to_json(input_file, filename, output_file, map_file):
        ...
        for filename in os.listdir(corpus_directory):
            ...
            with codecs.open(file_path, "r", encoding="utf-8") as inp:
                if PROCESS_AS_JSON_DOC[filename]:
                    fulldoc_to_json(input_file=inp, filename=filename, output_file=output_file, map_file=map_file)
                else:
