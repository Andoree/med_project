...
def main():
    ...
    pt_df = joined_df[joined_df["term_type"] == "PT"]
    unique_pt_codes = set(pt_df.ru_meddra_code.values)
    unique_cuis_of_pt_codes_set = set(pt_df.umls_cui.values)
    print(f"Unique pt meddra codes: {len(unique_pt_codes)}")
    print(f"Unique meddra_codes: {len(unique_meddra_codes)}")
    print(f"Unique pt / Unique meddra codes: {len(unique_pt_codes) / len(unique_meddra_codes)}")
    # Excluding CUIS that has a term with PT type from wordnet-lemma intersection
    all_covered_cuis_set = all_covered_cuis_set.difference(unique_cuis_of_pt_codes_set)
    ...
    meddra_pt_codes_df = meddra_pt_codes_df[meddra_pt_codes_df.term_type == "PT"]
    ...
    with open(output_stats_path, 'w+', encoding="utf-8") as output_file:
        output_file.write(f"Unique pt meddra codes: {len(unique_pt_codes)}\n")
        output_file.write(f"Unique meddra_codes: {len(unique_meddra_codes)}\n")
        output_file.write(f"Unique pt / Unique meddra codes: {len(unique_pt_codes) / len(unique_meddra_codes)}\n")
