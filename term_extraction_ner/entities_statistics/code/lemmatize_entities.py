...
def main():
    ...
    print('Модель загружена, начинаю лемматизацию')
    token_lemma_map = {}
    with codecs.open(entities_input_path, 'r', encoding='utf-8') as input_file, \
        ...
        for line in input_file:
            ...
            token = attrs[0]
            entity_type = attrs[1]
            if token_lemma_map.get(token) is not None:
                lemmatized_token = token_lemma_map[token]
            else:
                token = unify_sym(token)
                lemmatized_token = process_udpipe(process_pipeline, text=token, keep_pos=True, keep_punct=False)
                lemmatized_token = ' '.join(lemmatized_token)
                token_lemma_map[token] = lemmatized_token
